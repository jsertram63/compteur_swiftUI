...
class ChronometreViewModel: ObservableObject {
    private var sourceTimer: DispatchSourceTimer?
    private let queue = DispatchQueue(label: "stopwatch.timer")
    private var counter: Int = 0
    @Published var nombreTour: Int = 0
    ...
    private var currentLaps = [TourItem]() {
        didSet {
    ...
    func lap() {
        if let firstLap = self.laps.first {
            ...
            self.currentLaps.append(TourItem(count: self.counter, diff: difference))
        } else {
            self.currentLaps.append(TourItem(count: self.counter))
        }
    }
    ...
    private func updateTimer() {
        ...
        DispatchQueue.main.async {
            self.stopWatchTime = ChronometreViewModel.convertCountToTimeString(counter: self.counter)
        }
    }
